            ...
            <div class="card shadow-sm rounded" style="overflow: visible;">
                ...
                <div class="card-body">
                    <form wire:submit.prevent="addSupplier">
                        ...
                        <div class="form-group mb-2">
                            <label for="sup_name" class="form-label">Name <span class="text-danger">*</span></label>
                            <input type="text" name="sup_name" wire:model.live="sup_name" class="form-control form-control-sm rounded" id="sup_name" {{ $isView ? 'disabled' : ''}}>
                            @error('sup_name')
                                <p class="text-danger">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        @if(!$isView)
                        <div class="mt-3">
                            <button type="submit" class="btn btn-primary btn-sm">{{ $supplier ? 'Update' : 'Add' }}</button>
                        </div>
                        @endif
                    </form>
                </div>
