            ...
            <div class="col-md-11 m-auto">
                <div class="card shadow-sm">
                    <div class="card-header transaction-log-page-header">
                        <h5 class="fw-bold mb-0 list-page-unified-title">Manage Category</h5>
                    </div>
                            ...
                            <table class="table table-hover list-simple-table list-simple-table-compact master-data-list-table">
                                <thead>
                                    <tr>
                                        ...
                                        <th class="master-data-col-name">
                                            <button type="button" wire:click="sortBy('cat_name')" class="btn btn-sm p-0 border-0 bg-transparent text-dark text-decoration-none fw-semibold master-data-sort-btn">
                                                Category Name{{ $sortColumn === 'cat_name' ? ($sortOrder === 'asc' ? ' ↑' : ' ↓') : '' }}
                                            </button>
                                        </th>
                                ...
                                <tbody>
                                    @forelse($categories as $category)
                                        <tr>
                                            <td class="text-muted">{{ ($categories->firstItem() ?? 0) + $loop->index }}</td>
                                            <td>{{ $category->cat_name }}</td>
                                            <td class="text-end">
                                                <button wire:confirm="Are you sure you want to delete?" wire:click="deleteCategory({{ $category->id }})" type="button" class="btn btn-danger btn-sm" title="Delete"><i class="fas fa-trash-alt"></i></button>
                                            </td>
                                        </tr>
                                    @empty
                                        <tr>
