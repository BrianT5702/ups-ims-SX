...
class Transaction extends BaseModel
{
     ...
     *
     * 1. DO/PO document date (from joins), direction as requested (default DESC).
     * 2. Source doc number — numeric when possible (049050 before 049049 when DESC).
     * 3. Trimmed doc string (non-numeric PO refs).
     * 4. Row `created_at`, then `id`.
     */
    public function scopeOrderByLogDisplayDate(Builder $query, string $direction = 'desc'): Builder
    {
        ...
        $qualifiedDocNum = $query->qualifyColumn('source_doc_num');

        return $query
            ->orderByRaw("COALESCE(tx_log_do.date, tx_log_po.date, transactions.created_at) {$dir}")
            ->orderByRaw(
                "CAST(TRIM({$qualifiedDocNum}) AS UNSIGNED) {$tieDir}"
            )
            ->orderByRaw("TRIM({$qualifiedDocNum}) {$tieDir}")
            ->orderByRaw("{$qualifiedCreated} {$tieDir}")
